...
public final class TextDrawingArea extends Rasterizer2D {
	...
	private void method392(byte abyte0[], int i, int j, int k, int l, int i1) {
		...
		if (j < Rasterizer2D.yClipStart) {
			int j2 = Rasterizer2D.yClipStart - j;
			l -= j2;
			j = Rasterizer2D.yClipStart;
			i2 += j2 * k;
			j1 += j2 * Rasterizer2D.width;
		}
		if (j + l >= Rasterizer2D.yClipEnd)
			l -= ((j + l) - Rasterizer2D.yClipEnd) + 1;
		...
		if (i + k >= Rasterizer2D.xClipEnd) {
			int l2 = ((i + k) - Rasterizer2D.xClipEnd) + 1;
			k -= l2;
			l1 += l2;
			k1 += l2;
		}
	...
	private void method394(int i, int j, byte abyte0[], int k, int l, int i1, int j1) {
		...
		int i2 = 0;
		int j2 = 0;
		if (l < Rasterizer2D.yClipStart) {
			int k2 = Rasterizer2D.yClipStart - l;
			i1 -= k2;
			l = Rasterizer2D.yClipStart;
			j2 += k2 * k;
			k1 += k2 * Rasterizer2D.width;
		}
		if (l + i1 >= Rasterizer2D.yClipEnd)
			i1 -= ((l + i1) - Rasterizer2D.yClipEnd) + 1;
		if (j < Rasterizer2D.xClipStart) {
			int l2 = Rasterizer2D.xClipStart - j;
			k -= l2;
		...
		if (j + k >= Rasterizer2D.xClipEnd) {
			int i3 = ((j + k) - Rasterizer2D.xClipEnd) + 1;
			k -= i3;
			i2 += i3;
			l1 += i3;
		}
